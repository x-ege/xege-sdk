...
网格的运动遵循简化的弹性方程。对于每个内部点 $(i, j)$，其加速度由相邻点的位置决定：

$$
\begin{aligned}
a_x &= (x_{i-1,j} + x_{i+1,j} - 2x_{i,j}) + (x_{i,j-1} + x_{i,j+1} - 2x_{i,j}) \\
a_y &= (y_{i-1,j} + y_{i+1,j} - 2y_{i,j}) + (y_{i,j-1} + y_{i,j+1} - 2y_{i,j})
\end{aligned}
$$

这个公式实际上是二维拉普拉斯算子的离散形式，它计算了四个方向（上下左右）邻居对当前点的拉力总和。当相邻点离得越远，拉力就越大。
